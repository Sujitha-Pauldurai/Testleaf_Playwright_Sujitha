...
test("Sales Force application",async({ page }) => {
  ...
  await page.locator('#createLeadForm_firstName').fill("Suji")
  await page.locator('#createLeadForm_lastName').fill("Narayanan")
  ...
  await page.locator('#updateLeadForm_companyName').fill("Testleaf-B1")
  await page.locator('//input[@name="submitButton" and @value="Update"]').click()
  let updatedCompany = await page.locator('#viewLead_companyName_sp').innerText()
  if(updatedCompany.includes("Testleaf-B1")){
     console.log("Lead is Updated successfully");
  }
  else{
    console.log("Lead is not updated");
  } 
